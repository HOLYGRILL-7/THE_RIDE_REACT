...
const Home = () => {
  ...
  const [fullName, setFullName] = useState(""); // For signup form
  const [email, setEmail] = useState(""); // For both forms
  const [password, setPassword] = useState(""); // For both forms
  ...
  const handleFormSubmit = (e) => {
    ...
    navigate("/student-interface", { state: { fullName } });
; // Navigate to student interface on form submit, and send fullName as state
  };

  // Testing validation
  const TEST_FULLNAME = "Bismark Jones";
  const TEST_EMAIL = "bismarkjonesoduro@gmail.com";
  const TEST_PASSWORD = "fraser123";

  const validateForm = () => {
    const newErrors = {};
    // Basic validation for empty fields
    //Going to add my name and email as required fields and password as well so i'll change the fullName.trim() === "" to fullName.trim() !== Bismark Jones, same to email and password
    if (formType === "signup") {
      if (fullName.trim() === "") {
      ...
      else if (fullName.trim() !== TEST_FULLNAME) {
        newErrors.fullName = "Full name must match test credentials";
      }
    }

    if (email.trim().toLocaleLowerCase() !== TEST_EMAIL) {
    ...
    <div className="min-h-screen bg-blue-100 overflow-hidden relative">
      ...
      <div
        className={`absolute top-118 left-1/3 transform -translate-x-1/2 z-[60] transition-all duration-1000 ease-out ${
          animationStage >= 4
            ? "-translate-y-90" // Stage 4: Move to top for form
            : animationStage >= 3
            ? "-translate-y-16" // Stage 3: Lift up a bit
            : animationStage >= 2
            ? "-translate-x-1 opacity-100" // Stage 2: Center on screen
      ...
      >
        <form onSubmit={handleFormSubmit} className="p-8 max-w-md mx-auto">
            ...
            {formType === "signup" && (
              <>
                <div className="relative">
                  ...
                  />
                  {errors.fullName && (
                    <p className="absolute text-red-500 text-sm top-full left-3">
                      {errors.fullName}
                    </p>
                  )}
                </div>
      ...
      >
        {isReplaying ? (
          ...
          <img src={refresh} alt="Location" className="w-6 h-6" />
        )}
      </button>
    </div>
  );
